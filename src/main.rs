...
quick_main!(|| -> Result<()> {
    ...
    // Now rename input
    fs::rename(&args.input, &original_path).chain_err(|| {
        format!(
            "Unable to rename\n{}\nto\n{}",
            args.input.display(),
    ...
    let mut input_file = File::open(&original_path).chain_err(|| {
        format!("Can't open input file: {}", original_path.display())
    })?;

    // Now lets have the output handle ready to go
    // Which happens to be same as input path was
    let mut output_file = File::create(&args.input).chain_err(|| {
        format!("Can't create output file: {}", args.input.display())
    })?;

    // Read the input_file into a buffer
    let mut input_buffer = String::new();
    input_file.read_to_string(&mut input_buffer).chain_err(|| {
        format!("Can't read from input file: {}", original_path.display())
    })?;
    ...
    let dom = parse_fragment(RcDom::default(), opts, qualname!(html, "body"), Vec::new())
        .from_utf8()
        ...
        .unwrap();
    {
        ...
        let attr_tendril = Tendril::from_str("math display").expect("Can't make tendril");
